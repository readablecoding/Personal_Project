...
public class PageNavigator {
	...
	private int pagePerGroup;		//그룹당 페이지 수: 보여줄 페이지 번호 수
	...
	private int startPageGroup;		//현재 그룹의 첫 페이지: 그룹당 페이지 수가 5라면 그룹이 0일때 1, 그룹이 1일때 6
	private int endPageGroup;		//현재 그룹의 마지막 페이지: 전체 페이지 수에 따라 바뀜
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		//페이지당 글 수, 그룹당 페이지 수, 현재 페이지, 전체 글 수를 전달받음  -> 이 4개의 값으로 다 구할 수 있다.
		...
		//딱 떨어졌을 때를 대비해 -1를 함. int형 나누기는 소수점이 잘린다.  맨 마지막 글로 인해 페이지가 생기지 않게 만든다.

		//전달된 현재 페이지가 1보다 작으면 현재페이지를 1페이지로 지정
		if (currentPage < 1)	currentPage = 1;
		//0페이지로 갈 수 없다. 무조건 1페이지로 고정시킴
		
		//전달된 현재 페이지가 마지막 페이지보다 크면 현재페이지를 마지막 페이지로 지정
		if (currentPage > totalPageCount)	currentPage = totalPageCount;
		//계속 오른쪽 버튼을 눌러도 마지막페이지로 고정시킴
		
		this.currentPage = currentPage;

		//현재 그룹
		currentGroup = (currentPage - 1) / pagePerGroup;
		//나누기를 통해 소수점이 나오는데 int를 통해 그룹을 정해둠, 0그룹부터 시작함
		
		//현재 그룹의 첫 페이지
		startPageGroup = currentGroup * pagePerGroup + 1;
		//5개씩 보여준다면  0번째 그룹의 첫 페이지는 1이다. 1번째 그룹의 첫 페이지는 6이다.
		...
		//5개씩 보여준다면 0번째 그룹의 마지막페이지는 5가 되게끔 한다. 1번째 그룹의 마지막페이지는 10이 된다.
